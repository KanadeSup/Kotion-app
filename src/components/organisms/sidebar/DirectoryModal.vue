...
const props = defineProps({
   ...
   data: {
      type: Object as PropType<Directory | null>,
   },
});
...
async function onSubmit() {
   if (props.type === "Create") {
      const result = await fileSystemStore.add(props.data ? props.data.absolutePath : null, inputData.value.name, true);
      if (!result.isError) {
         open.value = false;
         return;
      }
      if (result.errorType == "Existed") {
         error.value = {
            errorInput: true,
            message: "The directory is already existed",
         };
         return;
      }
      console.error("Create directory was failed:", result)
      error.value = {
         errorInput: false,
   ...
   if (props.type === "Update" && props.data) {
      ...
      console.error("update directory was failed:", result)
      error.value = {
         errorInput: false,
         message: "Something went wrong, please try again",
      };
...
<template>
   <Dialog v-model:open="open">
      <DialogContent class="[&>button]:hidden flex flex-col items-center gap-1 px-7 w-auto">
         <h1 v-if="type == 'Create'">What is your directory name ?</h1>
         <h1 v-else>Update your directory</h1>

         <p class="text-xs text-gray-400">Enter your name and choose icon</p>
         <div class="flex items-center mt-3">
            <IconFolder class="w-[30px] h-[30px] rounded-l-md stroke-gray-300 border p-1" />
            <Input
         ...
         <div class="w-full mt-2">
            <Button class="w-full font-medium text-sm hover:bg-gray-300" @click="onSubmit">
               <p v-if="type === 'Create'">Create directory</p>
